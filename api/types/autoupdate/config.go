...
func checkAgentSchedules(c *autoupdate.AutoUpdateConfig) error {
	...
	for i, group := range groups {
		...
		if group.CanaryCount > MaxCanaryCount {
			return trace.BadParameter("spec.agents.schedule.regular[%d].canary_count cannot be greater than %d", i, MaxCanaryCount)
		}
		...
		if group.CanaryCount < 0 {
			return trace.BadParameter("spec.agents.schedule.regular[%d].canary_count cannot be negative", i)
		}
